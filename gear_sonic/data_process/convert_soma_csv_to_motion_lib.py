...
Usage:
    # G1 flat CSVs (default robot), 120 fps source -> 30 fps
    python gear_sonic/data_process/convert_soma_csv_to_motion_lib.py \
        --input /path/to/g1/csv \
        --output data/g1_motions.pkl --fps 30 --fps_source 120
...
def downsample_sequence(entry: dict, fps_source: int, fps_target: int) -> dict:
    ...
    if fps_source == fps_target:
        return entry
    if fps_source % fps_target != 0:
        # int() truncation would keep every `floor(ratio)`-th frame while
        # stamping fps_target on the entry — e.g. 120->50 keeps every 2nd frame
        # (really 60 fps) but labels it 50, so the clip plays 20% slow and its
        # timebase no longer matches SMPL sidecars. Refuse rather than corrupt.
        raise ValueError(
            f"fps_source={fps_source} is not an exact multiple of fps_target="
            f"{fps_target}; stride-based downsampling would mislabel the result. "
            f"Use an integral ratio (e.g. 120->30) or keep the source rate "
            f"(--fps {fps_source} --fps_source {fps_source}) and let the motion "
            f"lib resample at load."
        )
