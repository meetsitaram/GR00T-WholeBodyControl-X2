...
This is **Phase 0** of the X2 Ultra ONNX deploy plan: proving that the
...
def main():
    ...
    parser.add_argument(
        "--use-encoder", choices=("g1", "smpl"), default="g1",
        help="Encoder branch. 'smpl' drives the policy from a converted "
        "SMPL stream (--smpl-stream) through the trained smpl encoder — "
        "human-conditioned tracking with NO explicit retarget in the loop. "
        "smpl mode requires --compare-pt (the .pt drives; no smpl ONNX "
        "exists yet) and skips the ONNX session entirely.")
    parser.add_argument(
        "--smpl-stream", default=None,
        help="npz from gear_sonic.scripts.pico_tape_to_smpl_obs "
        "(smpl_joints_local + global_orient_quat_wxyz + fps).")
    args = parser.parse_args()

    smpl_data = None
    if args.use_encoder == "smpl":
        if args.compare_pt is None or args.smpl_stream is None:
            parser.error("--use-encoder smpl requires --compare-pt and "
                         "--smpl-stream")
        smpl_data = dict(np.load(args.smpl_stream))
        print(f"smpl mode: {args.smpl_stream} "
              f"({len(smpl_data['smpl_joints_local'])} frames @ "
              f"{float(smpl_data['fps']):.0f} Hz); ONNX session skipped.",
              flush=True)

    if args.use_encoder != "smpl" and args.onnx is None:
    ...
    if args.tuning == "__default__":
        ...
        default_yaml = (
            ...
            / "gear_sonic_deploy/configs/real_deploy_tuning/walk_101.yaml"
        )
    ...
    if args.compare_pt is not None:
        print(f"Loading .pt actor from {args.compare_pt} "
              f"(encoder={args.use_encoder}) ...", flush=True)
        pt_actor = load_actor_from_checkpoint(
            args.compare_pt, args.device, encoder=args.use_encoder)
        print("  .pt actor loaded.", flush=True)
    if args.compare_pt is not None and onnx_actor is not None:
    ...
    def step_once() -> str | None:
        ...
        if smpl_data is not None:
            from eval_x2_mujoco import build_smpl_tokenizer_obs
            tokenizer_obs = build_smpl_tokenizer_obs(
                smpl_data, motion_time, base_quat, motion_data, motion_fps,
                ori_mode=getattr(pt_actor, "ori_mode", "full"))
        else:
            tokenizer_obs = build_tokenizer_obs(
                motion_data, motion_time, base_quat, motion_fps)

        # ONNX action (skipped in smpl mode — no smpl ONNX exists yet).
        action_il_onnx = (onnx_actor(proprioception, tokenizer_obs)
                          if onnx_actor is not None else None)
        ...
        if (args.obs_dump is not None and step_count == 0
                and action_il_onnx is not None):
            import struct
        ...
        if pt_actor is not None:
            ...
            if parity_logger is not None and action_il_onnx is not None:
        ...
        if step_count % 250 == 0:
            extra = ""
            if action_il_pt is not None and action_il_onnx is not None:
                extra = (
