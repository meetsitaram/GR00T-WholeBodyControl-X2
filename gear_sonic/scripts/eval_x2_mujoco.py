...
Loads a .pt checkpoint, reconstructs the universal-token actor (g1 or
teleop encoder — see --use-encoder — + g1_dyn decoder), reads the
motion-lib PKL for reference commands, and runs a PD-control loop in
MuJoCo with on-screen rendering.
...
Usage:
    python gear_sonic/scripts/eval_x2_mujoco.py \\
        ...
        --motion gear_sonic/data/motions/x2_demo_bank.pkl
...
# -----------------------------------------------------------------------------
# ACTUATOR PLANT — imported, NOT duplicated.
#
# These used to be hardcoded dicts here. That made this file a THIRD independent
# copy of the plant (alongside x2_ultra.py and the MJCF), and on 2026-08-23 it
# had silently drifted: training moved to the vendor datasheet while this file
# still held the H2-derived estimates. Because the phi codec sources its X2
# action scales from THIS function, every exported ONNX would have carried
# pre-vendor scales -- a 1.5-2.3x command error on eight joints with no error
# raised anywhere.
#
# Selection order: $X2_PLANT > plant_config.ACTIVE_PLANT.
# To score a checkpoint trained BEFORE the vendor change, pin its own plant:
#     X2_PLANT=legacy_pre20260823 python gear_sonic/scripts/eval_x2_mujoco.py ...
# An A/B across the change MUST do this -- action_scale = 0.25*effort/kp, so a
# checkpoint scored under the wrong plant is mis-scaled by up to 2.86x.
#
# plant_config reads plain YAML and imports no isaaclab, so this stays usable in
# MuJoCo-only environments (a real import of x2_ultra needs a live sim app and
# dies with ModuleNotFoundError: carb).
_PLANT_DIR = (pathlib.Path(__file__).resolve().parents[1]
              / "envs" / "manager_env" / "robots")
if str(_PLANT_DIR) not in sys.path:
    sys.path.insert(0, str(_PLANT_DIR))
from plant_config import load_plant as _load_plant   # noqa: E402

PLANT = _load_plant()
#: substring -> value, first match wins; ORDER MATTERS (specific before general)
ARMATURES = PLANT.armature
EFFORT_LIMITS = PLANT.effort
...
_ANCHOR_ORI_MODE = os.environ.get("X2_ANCHOR_ORI_MODE", "b").strip().lower()
# Reproduce the pre-fix 6D channel-order bug (sim2sim_ablation_study.md
# section 6, commit 5e00de9) for footage / regression demos ONLY.
_ORI6D_REPRO_COLMAJOR_BUG = os.environ.get("X2_ORI6D_REPRO_COLMAJOR_BUG", "0") == "1"
if _ORI6D_REPRO_COLMAJOR_BUG:
    print("*** X2_ORI6D_REPRO_COLMAJOR_BUG=1: PRE-FIX column-major 6D flatten "
          "(the 2026-05-01 bug) IS ACTIVE -- demo/repro only ***", flush=True)
...
class UniversalTokenActor(nn.Module):
    ...
    inference path for a single encoder (g1 or teleop) + g1_dyn decoder + FSQ
    quantizer.
    ...
    Pipeline (matches universal_token_modules.encode + decode for g1/g1_dyn):
        tokenizer_obs(680 g1 / 267 teleop) ── encoder MLP ──► 64
    ...
    FSQ + decoder are encoder-agnostic: once the 64-dim latent is produced,
    everything downstream is identical for both encoders.
    ...
    def __init__(self, encoder_input_dim: int = 680):
        super().__init__()
        self.encoder = SimpleMLP([encoder_input_dim, 2048, 1024, 512, 512, 64])
...
# Encoder input dims per universal-token encoder name (see
# gear_sonic/config/exp/manager/universal_token/all_modes/sonic_x2_ultra.yaml
# `algo.config.actor.backbone.encoders.*.inputs`):
#   g1:     command_multi_future_nonflat (620) + motion_anchor_ori_b_mf_nonflat
#           (60) = 680
#   teleop: command_multi_future_lower_body (240) + vr_3point_local_target (9)
#           + vr_3point_local_orn_target (12) + motion_anchor_ori_b (6) = 267
#   smpl:   smpl_joints_multi_future_local_nonflat (720) +
#           smpl_root_ori_b_multi_future (60) +
#           joint_pos_multi_future_wrist_for_smpl (60) = 840
#           (10 frames x [joints 72 | root 6D 6 | X2 wrists 6]; window
#           spacing 0.02 s — HALF the g1 encoder's 0.1 s stride)
ENCODER_INPUT_DIMS = {"g1": 680, "teleop": 267, "smpl": 840}

# smpl-encoder window constants (sonic_release.yaml:50-51)
SMPL_NUM_FUTURE_FRAMES = 10
SMPL_DT_FUTURE = 0.02
# X2 wrist DOFs in IsaacLab order (ckpt config override; G1 default [23..28]
# is WRONG for X2 — bigrun bug #2)
X2_WRIST_IL_SLICE = slice(25, 31)


def build_smpl_tokenizer_obs(smpl_data, current_time, base_quat_wxyz,
                             motion_data, motion_fps, ori_mode="full"):
    """Build the 840-D smpl-encoder input from a converted SMPL stream.

    smpl_data: dict-like with ``smpl_joints_local`` (T,24,3) [slots 22/23 =
    thumb tips], ``global_orient_quat_wxyz`` (T,4) [y-up->z-up + base-rot
    removed, w-first], ``fps`` (50). Produced by
    ``gear_sonic.scripts.pico_tape_to_smpl_obs`` (gated conversion).

    Layout per frame f (t + f*0.02 s): [joints(72) | root_6d(6) | wrists(6)]
    -> (10, 84) -> 840. The root 6D is ROBOT-ANCHOR-relative
    (quat_inv(base) * smpl_root, mat[..., :2] flattened) — matching
    ``commands.py`` smpl_root_ori_b_multi_future; feeding world-frame 6D is
    the documented failure mode. Wrists come from the reference motion's
    IL-order dofs [25..31)."""
    joints = smpl_data["smpl_joints_local"]
    orient = smpl_data["global_orient_quat_wxyz"]
    smpl_fps = float(smpl_data["fps"])
    n_smpl = len(joints)
    m = _m(motion_data)
    ...
    if ori_mode == "heading":
        # v1.1 cores: normalize by robot YAW only (keeps human pitch/roll
        # relative to gravity). Full-orientation anchoring is silent OOD.
        v = cur_rot.apply([1.0, 0.0, 0.0])
        cur_rot = Rot.from_euler("z", np.arctan2(v[1], v[0]))
    frames = []
    for f in range(SMPL_NUM_FUTURE_FRAMES):
        t_f = current_time + f * SMPL_DT_FUTURE
        si = min(int(round(t_f * smpl_fps)), n_smpl - 1)
        term1 = joints[si].reshape(72)
        oq = orient[si]  # wxyz
        rel = cur_rot.inv() * Rot.from_quat([oq[1], oq[2], oq[3], oq[0]])
        term2 = rel.as_matrix()[:, :2].reshape(6)
        fi = min(int(t_f / dt), total_frames - 1)
        jpos_il = m["dof"][fi][IL_TO_MJ_DOF]
        term3 = jpos_il[X2_WRIST_IL_SLICE]
        frames.append(np.concatenate([term1, term2, term3]).astype(np.float32))
    return np.concatenate(frames).astype(np.float32)


def load_actor_from_checkpoint(ckpt_path: str, device: str = "cpu",
                               encoder: str = "g1"):
    # frozen-core T1 candidate: the frozen GEAR-SONIC G1 policy behind the Phi
    # codec (same call contract). Trigger: --checkpoint frozen-core[:<ckpt.pt>]
    if ckpt_path.startswith("smpl-g1:"):
        # NATIVE G1 whole-body (no codec): smpl encoder + g1_dyn decoder.
        #   smpl-g1:<release.pt>                  pure G1 core (v1.1 / v1)
        #   smpl-g1:<decoder_ckpt>:<release.pt>   fine-tune decoder + release
        from frozen_core_sonic_codec import SmplG1Actor
        parts = ckpt_path.split(":")[1:]
        return SmplG1Actor(
            decoder_ckpt=parts[0],
            release_ckpt=parts[1] if len(parts) > 1 else None,
            device=device)
    if ckpt_path.startswith("frozen-core-smpl:"):
        # frozen-G1 whole-body: release smpl encoder + merged X2 decoder.
        # Optional second path picks the release ckpt (e.g. v1.1):
        #   frozen-core-smpl:<decoder_ckpt>[:<release_ckpt>]
        from frozen_core_sonic_codec import FrozenCoreSmplActor
        parts = ckpt_path.split(":")[1:]
        return FrozenCoreSmplActor(
            merged_ckpt=parts[0],
            release_ckpt=parts[1] if len(parts) > 1 else None,
            device=device)
    if ckpt_path == "frozen-core" or ckpt_path.startswith("frozen-core:"):
        from frozen_core_sonic_codec import FrozenCoreG1SonicActor
        pt = ckpt_path.split(":", 1)[1] if ":" in ckpt_path else None
        return FrozenCoreG1SonicActor(ckpt_path=pt, device=device)
    ...
    actor = UniversalTokenActor(encoder_input_dim=ENCODER_INPUT_DIMS[encoder])
    enc_prefix = f"actor_module.encoders.{encoder}.module."
    new_sd = {}
    for k, v in sd.items():
        if k == "std":
            new_sd["std"] = v
        elif k.startswith(enc_prefix):
            new_sd[k.replace(enc_prefix, "encoder.module.")] = v
...
def build_tokenizer_obs(motion_data, current_time, base_quat_wxyz, motion_fps):
    ...
    for f in range(NUM_FUTURE_FRAMES):
        ...
        # ANCHOR-ORI CONVENTION (X2_ANCHOR_ORI_MODE, default "b" = unchanged).
        #   b       : normalize by the robot's FULL orientation (incl pitch/roll)
        #             -> motion_anchor_ori_b_mf, what the C++ deploy tokenizer
        #             builds and what pre-v1.1 models were trained on.
        #   heading : normalize by the robot's YAW ONLY, preserving the
        #             reference's pitch/roll vs gravity -> motion_anchor_ori_
        #             heading_mf, what every v1.1-lineage model expects.
        # Same 60-D shape either way, so a mismatch is silent. They coincide
        # when the robot is upright and diverge with its tilt.
        if _ANCHOR_ORI_MODE == "heading":
            # MATCH TRAINING EXACTLY: commands.py::anchor_heading_quat ->
            # torch_transform.get_heading_q, which is a TWIST extraction --
            # zero the x,y quaternion components and renormalize -- NOT an
            # Euler-yaw decomposition. This previously used
            # ``as_euler("zyx")[0]``, a THIRD convention (scipy lowercase is
            # extrinsic), which agreed with neither training nor the C++
            # deploy. All three coincide at zero tilt and diverge with it, so
            # the discrepancy was invisible in every upright test.
            _q = cur_rot.as_quat()            # xyzw
            _n = float(np.hypot(_q[3], _q[2]))
            _ref = (Rot.identity() if _n < 1e-12
                    else Rot.from_quat([0.0, 0.0, _q[2] / _n, _q[3] / _n]))
        else:
            _ref = cur_rot
        relative = _ref.inv() * future_rot
        ...
        if _ORI6D_REPRO_COLMAJOR_BUG:
            # MEDIA/REPRO ONLY (2026-09-09): re-create the pre-2026-05-01
            # column-major flatten so the documented collapse can be
            # re-rendered. Never on by default; loud at startup.
...
# ---------- Teleop-encoder observation construction ----------
# The teleop encoder consumes 267 dims, concatenated in the exact order the
# universal-token module declares its inputs (sonic_x2_ultra.yaml
# `encoders.teleop.inputs`; UniversalTokenModule._encode_single does a plain
# torch.cat over the declared terms):
#   command_multi_future_lower_body (240)
#   vr_3point_local_target          (9)
#   vr_3point_local_orn_target      (12)  <- quaternions (3 x wxyz), NOT 6D
#   motion_anchor_ori_b             (6)
#
# All reference quantities come from the checkpoint's TrackingCommand config
# (softland_173528/config.yaml):
#   anchor_body:          pelvis
...
# vr_3point_body_pos_w (commands.py) = FK body pos + quat_apply(body_quat, offset).
TELEOP_ANCHOR_BODY = "pelvis"
TELEOP_VR_3POINT_BODIES = [
    "left_wrist_roll_link", "right_wrist_roll_link", "torso_link",
]
TELEOP_VR_3POINT_OFFSETS = np.array([
    [0.18, -0.025, 0.0],
    [0.18, 0.025, 0.0],
    [0.0, 0.0, 0.35],
], dtype=np.float64)

# Lower-body joint indices. TrackingCommand.__init__ (commands.py:177) builds
#   lower_joint_isaaclab_indices = [mujoco_to_isaaclab_dof[i] for i in range(12)]
# i.e. the IsaacLab indices of MuJoCo joints 0..11, and gathers the IL-order
# reference dof with them — which lands the 12 lower-body joints back in
# MuJoCo order (left hip p/r/y, knee, ankle p/r; then right side). Since the
# PKL stores dof in MuJoCo order, gathering IL-order dof by these indices is
# identical to taking dof[:12] straight from the PKL. We keep the explicit
# index chain for provenance.
LOWER_BODY_IL_IDX = MJ_TO_IL_DOF[:12]
assert [IL_TO_MJ_DOF[i] for i in LOWER_BODY_IL_IDX] == list(range(12))


def _quat_mul_wxyz(q1, q2):
    """Hamilton product q1 ⊗ q2, both wxyz (matches IsaacLab quat_mul)."""
    w1, x1, y1, z1 = q1
    w2, x2, y2, z2 = q2
    return np.array([
...
def _quat_conj_wxyz(q):
    """Conjugate (= inverse for unit quats), wxyz."""
    return np.array([q[0], -q[1], -q[2], -q[3]])


class TeleopObsBuilder:
    """Builds the 267-dim teleop tokenizer obs from the reference motion.

    Reference link world poses come from MuJoCo FK: a scratch ``MjData`` is
    set to the reference frame's qpos (root pos + wxyz quat + MuJoCo-order
    dof) and ``mj_kinematics`` gives body xpos/xquat/xmat for the anchor and
    the 3 VR bodies — the same kinematic chain the motion lib's FK uses
    (both read this MJCF), so poses match IsaacLab's ``get_body_pos_w`` /
    ``get_body_quat_w``. env_origins cancel in every term (all differences).
    """

    def __init__(self, mj_model):
        self._model = mj_model
        self._data = mujoco.MjData(mj_model)  # scratch, never stepped
        self._anchor_bid = mj_model.body(TELEOP_ANCHOR_BODY).id
        self._vr_bids = [mj_model.body(n).id for n in TELEOP_VR_3POINT_BODIES]

    def _fk(self, m, fi):
        """Run FK at motion frame ``fi``; returns (anchor_pos, anchor_quat_wxyz,
        vr_pos (3,3) with offsets applied, vr_quat_wxyz (3,4))."""
        d = self._data
        d.qpos[0:3] = np.asarray(m["root_trans_offset"][fi], dtype=np.float64)
        # PKL root_rot is xyzw (scipy); MuJoCo qpos[3:7] is wxyz.
        q = np.asarray(m["root_rot"][fi], dtype=np.float64)
        d.qpos[3:7] = [q[3], q[0], q[1], q[2]]
        d.qpos[7:7 + NUM_DOFS] = np.asarray(m["dof"][fi], dtype=np.float64)
        mujoco.mj_kinematics(self._model, d)

        anchor_pos = d.xpos[self._anchor_bid].copy()
        anchor_quat = d.xquat[self._anchor_bid].copy()  # wxyz
        vr_pos = np.stack([
            d.xpos[bid] + d.xmat[bid].reshape(3, 3) @ TELEOP_VR_3POINT_OFFSETS[i]
            for i, bid in enumerate(self._vr_bids)
        ])
        vr_quat = np.stack([d.xquat[bid].copy() for bid in self._vr_bids])
        return anchor_pos, anchor_quat, vr_pos, vr_quat

    def build(self, motion_data, current_time, base_quat_wxyz, motion_fps):
        """Build the 267-dim teleop encoder input.

        Args:
            motion_data: single-clip motion-lib dict.
            current_time: motion-clock time (s) of the current control step.
            base_quat_wxyz: the ROBOT's current pelvis quaternion from MuJoCo
                (used only by motion_anchor_ori_b, which is reference-vs-robot).
            motion_fps: motion clip fps.
        """
        m = _m(motion_data)
        total_frames = m["dof"].shape[0]
        dt = 1.0 / motion_fps
        ...
        # -- command_multi_future_lower_body (240) --
        # joint_pos_lower_body_multi_future is (F, 12) flattened frame-major
        # (120), then joint_vel likewise (120), concatenated pos-then-vel
        # (commands.py command_multi_future_lower_body). Future window starts
        # at the CURRENT frame, same as build_tokenizer_obs. Velocities use
        # the same backward finite difference as the g1 path.
        jpos_frames = []
        jvel_frames = []
        for f in range(NUM_FUTURE_FRAMES):
            future_time = current_time + f * DT_FUTURE_REF
            ...
            jpos_il = m["dof"][fi][IL_TO_MJ_DOF]
            jpos_frames.append(jpos_il[LOWER_BODY_IL_IDX].astype(np.float32))

            prev_fi = max(0, fi - 1)
            jvel_mj = (m["dof"][fi] - m["dof"][prev_fi]) * motion_fps
            jvel_il = jvel_mj[IL_TO_MJ_DOF]
            jvel_frames.append(jvel_il[LOWER_BODY_IL_IDX].astype(np.float32))

        command_lower = np.concatenate(
            [np.concatenate(jpos_frames), np.concatenate(jvel_frames)])  # (240,)

        # -- reference FK at the current frame --
        fi_now = min(int(current_time / dt), total_frames - 1)
        anchor_pos, anchor_quat, vr_pos, vr_quat = self._fk(m, fi_now)

        # -- vr_3point_local_target (9) --
        # observations.py::vr_3point_local_target: quat_apply(quat_inv(
        # anchor_quat_w), vr_3point_body_pos_w - anchor_pos_w). Purely
        # reference-side (both the points and the frame are the ref motion's).
        vr_local = np.stack([
            quat_rotate_inverse(anchor_quat, vr_pos[i] - anchor_pos)
            for i in range(len(self._vr_bids))
        ]).reshape(-1).astype(np.float32)  # (9,)

        # -- vr_3point_local_orn_target (12) --
        # observations.py::vr_3point_local_orn_target returns RAW quaternions
        # (wxyz, IsaacLab convention): quat_mul(quat_inv(anchor_quat_w),
        # vr_3point_body_quat_w) — 3 bodies x 4 = 12, NOT a 6D encoding.
        # Quaternion sign is deterministic: the ref root-quat sign cancels in
        # conj(anchor) ⊗ body since both contain it as a left factor.
        vr_orn = np.stack([
            _quat_mul_wxyz(_quat_conj_wxyz(anchor_quat), vr_quat[i])
            for i in range(len(self._vr_bids))
        ]).reshape(-1).astype(np.float32)  # (12,)

        # -- motion_anchor_ori_b (6) --
        # observations.py:970: subtract_frame_transforms(robot_anchor,
        # ref_anchor) -> ori = quat_inv(robot_quat) * ref_quat, then
        # matrix_from_quat(ori)[..., :2] flattened ROW-major
        # [m00,m01,m10,m11,m20,m21] — same 6D convention as the g1 path.
        # No heading removal; frame = the robot's current pelvis orientation
        # (use_ref_motion_root_quat_w_as_anchor is False in training).
        cur_rot = Rot.from_quat([base_quat_wxyz[1], base_quat_wxyz[2],
                                 base_quat_wxyz[3], base_quat_wxyz[0]])
        ref_rot = Rot.from_quat([anchor_quat[1], anchor_quat[2],
                                 anchor_quat[3], anchor_quat[0]])
        rel_mat = (cur_rot.inv() * ref_rot).as_matrix()
        anchor_ori_b = rel_mat[:, :2].reshape(-1).astype(np.float32)  # (6,)

        obs = np.concatenate([command_lower, vr_local, vr_orn, anchor_ori_b])
        assert obs.shape == (267,), (
            f"teleop obs is {obs.shape}, expected (267,) = "
            f"{command_lower.shape[0]}+{vr_local.shape[0]}+"
            f"{vr_orn.shape[0]}+{anchor_ori_b.shape[0]}")
        return obs.astype(np.float32)


# ---------- Main ----------
def _ladder_finish(ladder, args) -> None:
    """Dump push-ladder results + first-step/first-fall summary."""
    import json as _json
    res = ladder["results"]
    if args.push_ladder_out:
        with open(args.push_ladder_out, "w") as fh:
            for r in res:
                fh.write(_json.dumps(r) + "\n")
        print(f"[ladder] wrote {len(res)} pulses to {args.push_ladder_out}",
              flush=True)
    print("\n[ladder] ===== SUMMARY =====", flush=True)
    for d in ("fwd", "back"):
        rows = [r for r in res if r["dir"] == d]
        f_step = next((r["force"] for r in rows if r["stepped"]), None)
        f_fall = next((r["force"] for r in rows if r["fell"]), None)
        held = max((r["force"] for r in rows
                    if not r["stepped"] and not r["fell"]), default=0)
        print(f"[ladder] {d:>4}: held-without-step up to {held:.0f}N | "
              f"first step at {f_step if f_step is not None else '>max'}N | "
              f"first fall at {f_fall if f_fall is not None else '>max'}N",
              flush=True)
...
def main():
    ...
    motion_grp.add_argument(
        ...
        help="Multi-clip motion-lib PKL: iterate EVERY clip one-by-one in one "
             ...
             "robot falls. Mutually exclusive with --motion.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="With --motions, only play clips whose name CONTAINS this substring "
             "(case-insensitive). E.g. --clip right_wave_001 plays just "
             "that one gesture; --clip wave plays every wave variant.")
    parser.add_argument(
        "--record", default=None,
    ...
    parser.add_argument(
        "--use-encoder", choices=("g1", "teleop"), default="g1",
        help="Which universal-token encoder to drive the policy with. "
             "'g1' (default) = full-body 680-dim reference command (current "
             "behavior). 'teleop' = 267-dim VR-teleop encoder: lower-body "
             "multi-future command + 3-point (wrists+torso) targets in the "
             "reference anchor frame + anchor orientation. FSQ + decoder are "
             "shared, so everything downstream of the 64-dim latent is "
             "unchanged.")
    ...
    parser.add_argument(
        "--push-ladder", default="",
        help="Scripted push protocol 'start,step,max' in Newtons (e.g. "
             "'10,10,150'): after a 3s settle, apply --push-duration pulses "
             "at --push-body, alternating forward/backward per force level, "
             "observing 1.5s after each. Records first-step and first-fall "
             "forces per direction; exits when the ladder completes. "
             "Robustness benchmark mode (robustness_run_spec.md).")
    parser.add_argument(
        "--push-ladder-out", default="",
        help="JSONL path for per-pulse push-ladder results.")
    parser.add_argument(
        "--real-deploy-tuning", default="",
        help="Path to a gear_sonic_deploy/configs/real_deploy_tuning/*.yaml "
             "preset. REPLACES the sim-only DEPLOYMENT_*_SCALE gains with the "
             "preset's training-PD x trim, and applies the C++ safety stack's "
             "per-group target-dev clamps + target LPFs in the control loop "
             "(load_deploy_tuning).")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Sustained external ASSIST force (N) applied every step at --push-body "
             ...
             "Default 0 (off).")
    # ---- WAIST TITRATION RIG (2026-08-24) ---------------------------------
    # Reproduce the ROBOT's waist threshold in sim. On hardware, bending the
    # waist by hand past ~3 deg flips the commanded waist_pitch to the -25.8 deg
    # clamp rail and HOLDS it there (86% of ticks at 3-6 deg, 100% past 6 deg),
    # producing a violent fight. The existing --push-ladder could not reproduce
    # it: a linear force makes the robot STEP (graceful, waist_vel_rms rising
    # smoothly 0.012 -> 1.19 over 10-110 N) instead of isolating the waist.
    #
    # These two flags isolate it the way a hand does:
    #   --push-torque  applies a MOMENT at --push-body (neck/head) about the
    #                  pitch axis, rotating the torso over the waist rather
    #                  than shoving the whole robot.
    #   --pin-pelvis   freezes the floating base each step, so the robot cannot
    #                  step or fall out of the disturbance. All of the moment
    #                  goes into the waist joint, which is what a hand on the
    #                  torso actually does.
    # Both default OFF -- with neither set, behaviour is bit-identical to before.
    parser.add_argument(
        "--push-torque", type=float, default=0.0,
        help="Sustained MOMENT (N*m) at --push-body about the pitch axis, "
             "ramped linearly to this value over --torque-ramp seconds. "
             "Rotates the torso over the waist. Default 0 (off).")
    parser.add_argument(
        "--torque-ramp", type=float, default=20.0,
        help="Seconds over which --push-torque ramps 0 -> full (titration). "
             "Default 20.")
    parser.add_argument(
        "--torque-axis", default="pitch", choices=["pitch", "roll", "yaw"],
        help="Axis of the applied moment. pitch/roll bend the torso over the "
             "waist; YAW twists about the vertical -- use with --push-body pelvis "
             "to rotate the HIPS/base and test heading-deviation recovery, which "
             "is a different failure mode from waist bend. Default pitch.")
    parser.add_argument(
        "--hold-upright", action="store_true",
        help="Keep the base UPRIGHT (zero roll+pitch) every step while leaving "
             "height, translation and YAW free. The robot cannot tip over, but it "
             "still bears its own weight, can step, and can turn -- so the legs "
             "behave normally. Use this to study a disturbance without the robot "
             "falling out of it. REPLACES the old --pin-pelvis, which froze "
             "qpos[0:7] entirely: that left gravity and contact forces nowhere to "
             "go, the knees buckled and the torso sagged, and any result from it "
             "was an artefact of the collapsed pose rather than the disturbance "
             "(observed 2026-08-24).")
    ...
    deploy_tune = None
    tune_lpf_y = None
    if args.real_deploy_tuning:
        deploy_tune = load_deploy_tuning(args.real_deploy_tuning)
        KP = deploy_tune["kp"].astype(np.float64)   # replaces, never stacks
        KD = deploy_tune["kd"].astype(np.float64)
        tune_lpf_y = DEFAULT_DOF.copy().astype(np.float64)
        print(f"Real-deploy tuning ACTIVE: {args.real_deploy_tuning} "
              f"(PD replaced, dev clamps + LPFs on; action_clip="
              f"{deploy_tune['action_clip']:.0f})", flush=True)

    print(f"Loading actor from {args.checkpoint} ...", flush=True)
    actor = load_actor_from_checkpoint(args.checkpoint, args.device,
                                       encoder=args.use_encoder)
    print(f"  Actor loaded ({args.use_encoder} encoder, "
          f"{ENCODER_INPUT_DIMS[args.use_encoder]}-dim tokenizer obs).",
          flush=True)

    # ---- Build the clip list: single (--motion) or multi (--motions) ----
    ...
    push_dir_sign = 1.0
    # ---- scripted push ladder (robustness benchmark) ----
    ladder = None
    if args.push_ladder:
        _l0, _ld, _lm = (float(v) for v in args.push_ladder.split(","))
        _feet = []
        for cand in ("left_ankle_roll_link", "right_ankle_roll_link",
                     "left_ankle_link", "right_ankle_link"):
            try:
                _feet.append(mj_model.body(cand).id)
            except Exception:
                pass
        if len(_feet) < 2:
            raise SystemExit(f"push-ladder: cannot resolve foot bodies "
                             f"(found {len(_feet)})")
        ladder = dict(force=_l0, step=_ld, fmax=_lm, dir=+1.0,
                      state="settle", t_state=0.0, feet=_feet[:2],
                      feet_pre=None, grav_min=0.0, results=[])
        print(f"[ladder] {_l0:.0f}->{_lm:.0f}N step {_ld:.0f}, pulse "
              f"{args.push_duration:.2f}s on {args.push_body}", flush=True)
    teleop_obs_builder = (TeleopObsBuilder(mj_model)
                          if args.use_encoder == "teleop" else None)
    if teleop_obs_builder is not None:
        print("  Teleop tokenizer obs: 240 (lower-body cmd) + 9 (vr_3point pos) "
              "+ 12 (vr_3point quat) + 6 (anchor ori 6D) = 267", flush=True)
    push_steps_left = 0
    ...
    def reset_state(reason=""):
        ...
        if hasattr(actor, "reset"):
            actor.reset()   # frozen-core wrapper: clear G1-native action history
        if tune_lpf_y is not None:
            tune_lpf_y[:] = DEFAULT_DOF   # clear filter memory across resets
        _apply_init_state()
        episode_count += 1
    ...
    with mujoco.viewer.launch_passive(
        ...
        while viewer.is_running():
            ...
            if teleop_obs_builder is not None:
                tokenizer_obs = teleop_obs_builder.build(
            ...
            if deploy_tune is not None:
                tune_lpf_y += deploy_tune["lpf_alpha"] * (target_pos - tune_lpf_y)
                target_pos = DEFAULT_DOF + np.clip(
                    tune_lpf_y - DEFAULT_DOF,
                    -deploy_tune["max_target_dev"],
                    deploy_tune["max_target_dev"])
            ...
            # -- Scripted push ladder driver (robustness benchmark) --
            if ladder is not None and not paused:
                ladder["t_state"] += CONTROL_DT
                if ladder["state"] == "settle" and ladder["t_state"] >= 3.0:
                    ladder["feet_pre"] = [mj_data.xpos[f][:2].copy()
                                          for f in ladder["feet"]]
                    ladder["feet_z0"] = [float(mj_data.xpos[f][2])
                                         for f in ladder["feet"]]
                    ladder["airborne"] = [False, False]
                    ladder["air_ticks"] = [0, 0]
                    ladder["n_steps"] = 0
                    ladder["grav_min"] = 0.0
                    ladder["tilt_samp"] = []
                    ladder["waist_vel_samp"] = []
                    push_dir_sign = ladder["dir"]
                    push_force = ladder["force"]
                    push_steps_left = max(1, int(round(args.push_duration / CONTROL_DT)))
                    ladder["state"], ladder["t_state"] = "observe", 0.0
                    print(f"[ladder] {'FWD' if ladder['dir'] > 0 else 'BACK'} "
                          f"{ladder['force']:.0f}N", flush=True)
                elif ladder["state"] == "observe":
                    ladder["grav_min"] = min(ladder["grav_min"], -abs(float(gravity[2]) + 1.0))
                    # waist wobble / tilt (operator metric 2026-08-10):
                    # tilt from gravity_body, waist joint-vel RMS (dofs 12-14
                    # in MJ order = waist_yaw/pitch/roll).
                    _tilt = float(np.degrees(np.arccos(
                        np.clip(-float(gravity[2]), -1.0, 1.0))))
                    ladder["tilt_samp"].append(_tilt)
                    ladder["waist_vel_samp"].append(
                        float(np.sqrt(np.mean(
                            mj_data.qvel[6 + 12:6 + 15] ** 2))))
                    for fi, fb in enumerate(ladder["feet"]):
                        up = (float(mj_data.xpos[fb][2])
                              - ladder["feet_z0"][fi]) > 0.02
                        if up:
                            ladder["air_ticks"][fi] += 1
                            if (not ladder["airborne"][fi]
                                    and ladder["air_ticks"][fi] >= 3):
                                ladder["airborne"][fi] = True
                                ladder["n_steps"] += 1
                        else:
                            ladder["air_ticks"][fi] = 0
                            ladder["airborne"][fi] = False
                    if ladder["t_state"] >= 1.5 + args.push_duration:
                        disp = max(float(np.linalg.norm(
                            mj_data.xpos[f][:2] - p0))
                            for f, p0 in zip(ladder["feet"], ladder["feet_pre"]))
                        _ts = ladder["tilt_samp"] or [0.0]
                        _ws = ladder["waist_vel_samp"] or [0.0]
                        r = dict(force=ladder["force"],
                                 dir=("fwd" if ladder["dir"] > 0 else "back"),
                                 stepped=bool(disp > 0.07),
                                 n_steps=int(ladder["n_steps"]),
                                 foot_disp=round(disp, 3),
                                 tilt_max_deg=round(max(_ts), 2),
                                 tilt_std_deg=round(float(np.std(_ts)), 2),
                                 waist_vel_rms=round(float(np.mean(_ws)), 3),
                                 fell=False)
                        ladder["results"].append(r)
                        print(f"[ladder]   -> {'STEP' if r['stepped'] else 'held'} "
                              f"(n_steps {ladder['n_steps']}, "
                              f"foot disp {disp * 100:.1f}cm)", flush=True)
                        ladder["state"], ladder["t_state"] = "advance", 0.0
                if ladder["state"] == "advance":
                    if ladder["dir"] > 0:
                        ladder["dir"] = -1.0
                    else:
                        ladder["dir"] = +1.0
                        ladder["force"] += ladder["step"]
                    if ladder["force"] > ladder["fmax"]:
                        _ladder_finish(ladder, args)
                        break
                    ladder["state"], ladder["t_state"] = "settle", 0.0
            ...
            if push_steps_left > 0:
                ...
                if nrm > 1e-6:
                    fwd_w /= nrm
                mj_data.xfrc_applied[push_body_id, :3] = (push_force * push_dir_sign) * fwd_w
            else:
                mj_data.xfrc_applied[push_body_id, :3] = 0.0

            # -- WAIST TITRATION: moment at the neck + pinned base ------------
            if args.push_torque != 0.0:
                _t = sim_time
                _frac = min(1.0, max(0.0, _t / max(args.torque_ramp, 1e-6)))
                _mag = args.push_torque * _frac
                # world-frame axis: pitch = about +Y, roll = about +X
                _ax = ({"pitch": np.array([0.0, 1.0, 0.0]),
                        "roll":  np.array([1.0, 0.0, 0.0]),
                        "yaw":   np.array([0.0, 0.0, 1.0])})[args.torque_axis]
                mj_data.xfrc_applied[push_body_id, 3:6] = _mag * _ax
            if args.hold_upright:
                # Zero roll+pitch, PRESERVE yaw. Height/translation untouched, so
                # the legs still carry the load; yaw free, so the robot can still
                # turn (needed for the yaw-disturbance tests).
                _q = mj_data.qpos[3:7]           # w, x, y, z
                _yaw = math.atan2(2.0 * (_q[0] * _q[3] + _q[1] * _q[2]),
                                  1.0 - 2.0 * (_q[2] * _q[2] + _q[3] * _q[3]))
                mj_data.qpos[3] = math.cos(_yaw * 0.5)
                mj_data.qpos[4] = 0.0
                mj_data.qpos[5] = 0.0
                mj_data.qpos[6] = math.sin(_yaw * 0.5)
                mj_data.qvel[3] = 0.0            # kill roll rate
                mj_data.qvel[4] = 0.0            # kill pitch rate
                # qvel[5] (yaw rate) and qvel[0:3] (linear) left alone
            ...
            if reset_reason is not None:
                ...
                if ladder is not None:
                    # A fall during/after a pulse scores the CURRENT rung.
                    if ladder["state"] == "observe":
                        r = dict(force=ladder["force"],
                                 dir=("fwd" if ladder["dir"] > 0 else "back"),
                                 stepped=True,
                                 n_steps=int(ladder.get("n_steps", -1)),
                                 foot_disp=None, fell=True)
                        ladder["results"].append(r)
                        print(f"[ladder]   -> FELL ({reset_reason})", flush=True)
                        if ladder["dir"] > 0:
                            ladder["dir"] = -1.0
                        else:
                            ladder["dir"] = +1.0
