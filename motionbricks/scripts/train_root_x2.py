...
def main() -> None:
  ...
  parser.add_argument(
    "--include-pattern", action="append", default=None,
    help="Regex applied to (windowed) clip keys; repeatable. Overrides "
    "--filter presets. On a cache HIT the filter applies to manifest keys, "
    "so __winN suffixes are matchable (surgical corpus selection).",
  )
  parser.add_argument(
    "--exclude-pattern", action="append", default=None,
    help="Regex to drop (windowed) clip keys; repeatable. See --include-pattern.",
  )
  parser.add_argument(
    "--include-keys-pkl", type=Path, default=None,
    help="Pickle of an explicit (windowed) key whitelist; composed AND-wise "
    "with the pattern filters. For corpus selections regex cannot express "
    "(e.g. seeded random window subsets).",
  )
