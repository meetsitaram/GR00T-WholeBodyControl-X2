...
#ifndef AGI_X2_ZMQ_PACKED_MESSAGE_SUBSCRIBER_HPP
#define AGI_X2_ZMQ_PACKED_MESSAGE_SUBSCRIBER_HPP
  ...
  public:
    ...
    ZMQPackedMessageSubscriber(
      ...
      : host_(host), port_(port), topic_(topic), timeout_ms_(timeout_ms), verbose_(verbose),
        conflate_(conflate), rcv_hwm_(rcv_hwm), bind_(bind),
    ...
    bool Connect() {
      ...
      try {
        ...
        // bind_: this SUB is the well-known endpoint and publishers connect
        // to it (the laptop intent sender PUB-connects, as it did to the
        // Python token service's bound SUB; 2026-09-04).
        if (bind_) socket_->bind(endpoint); else socket_->connect(endpoint);
...
#endif // AGI_X2_ZMQ_PACKED_MESSAGE_SUBSCRIBER_HPP
