...
#             x2_pose_watchdog.py        Single-input fallback watchdog.
...
#
#     gear_sonic/utils/{teleop,pose_pipeline,planner}/   python modules the PC2-side
#     planner_stack/gear_sonic/utils/...  scripts import (pad bridge, token service,
#                                        kplanner hand retarget); py files only.
#     estop_gesture.py                   beside pad_locomotion_bridge.py (standalone import)
#     x2_pc2/                            systemd units + their install_*_service.sh
#     planner_stack/models/
#         dances_x2m2/<key>.x2m2         demo bank (gear_sonic_deploy/data/motions_x2m2/demo_bank/)
#         kplanner_idle_anchor_g1teleop_v3.pkl
#         x2_planner_primitives.pkl      lean/crouch/torso primitives for pc2_kplanner_onnx.py
#                                        (regenerated by tools/build_demo_bank_from_upstream.sh)
#         planner_onnx/                  planner graphs (pushed by push_to_pc2.sh)
#
#     The planner / pad scripts themselves (pc2_kplanner_onnx.py,
#     pad_locomotion_bridge.py, pc2_pad_daemon.py, x2_pad_autopair.py, the
#     token service) and the ONNX models go through x2_pc2/push_to_pc2.sh
#     (md5-gated, backed up) -- see x2_pc2/push_manifest.txt.
#
...
#     pc2_bringup.sh --pc2-host <PC2_IP> --prefix /home/run/gear-sonic
#
# Flags:
#     --pc2-host HOST            REQUIRED (or env PC2_HOST); no default
...
#                                (default: stage the shipped set from
#                                gear_sonic_deploy/models/, see step 9)
...
#     --skip-model               ignore --model and don't rsync any onnx (not
#                                even the shipped default set)
...
#     --force-pip                re-run the venv pip install (pyzmq onnxruntime
#                                joblib 'numpy<2' scipy pyyaml pygame msgpack).
#                                Default: skip when every dep already imports,
...
if [[ -z "${PC2_HOST}" ]]; then
    fail "PC2_HOST is not set. Pass --pc2-host <PC2_IP> or export PC2_HOST (x2_discover_network.sh writes ~/.x2/env.wired with it)."
fi
...
ssh_pc2 "mkdir -p \
    ...
    '${PC2_PREFIX}/gear_sonic/utils' \
    '${PC2_PREFIX}/planner_stack/gear_sonic/utils' \
    '${PC2_PREFIX}/planner_stack/models/dances_x2m2' \
    '${PC2_PREFIX}/planner_stack/models/planner_onnx' \
    '${PC2_PREFIX}/x2_pc2' \
    '${PC2_PREFIX}/data' \
    '${PC2_PREFIX}/policies' \
    '${PC2_PREFIX}/log'"
log "ensured ${PC2_PREFIX}/{onnxruntime,venv,ws,gear_sonic_deploy,gear_sonic,planner_stack,x2_pc2,data,policies,log} parents exist"
...
if [[ "${SKIP_VENV}" -eq 1 ]]; then
    ...
    # Python deps the PC2-side chain needs beyond system-site-packages:
    #   pyzmq        every daemon (pose wire, pad bridge, token service)
    #   onnxruntime  pc2_kplanner_onnx.py planner backend (+ ritual pairing check)
    #   joblib       kplanner idle-anchor pkl loader
    #   numpy/scipy  planner math, smpl_obs / token service rotations
    #   pyyaml       plant / tuning yaml readers
    #   pygame       pc2_pad_daemon.py / pad bridge controller input
    #   msgpack      merger / token wire framing
    # If every one already imports we skip pip entirely so re-runs of
    # bringup don't depend on PC2 having outbound HTTPS to pypi (a lab wifi
    # proxy can stall TLS, and the SDK ethernet path has no internet at
    # all). Internet is only required on the first install / after
    # --force-venv. --force-pip re-runs the install regardless.
    # numpy<2: the Orin image ships scipy 1.8 (system site, built against numpy 1.x) and a
    # numpy 2.x in ~/.local; with --system-site-packages pip would otherwise install
    # neither into the venv and scipy.spatial fails with "numpy.dtype size changed".
    PC2_PIP_DEPS="pyzmq onnxruntime joblib 'numpy<2' scipy pyyaml pygame msgpack"
    PC2_PIP_PROBE='import zmq, onnxruntime, joblib, numpy, scipy, scipy.spatial, yaml, pygame, msgpack, sys; assert int(numpy.__version__.split(".")[0]) < 2, numpy.__version__; sys.stdout.write("ok")'
    DEPS_PRESENT="$(ssh_pc2_capture "'${PC2_PREFIX}/venv/bin/python3' -c '${PC2_PIP_PROBE}' 2>/dev/null" || true)"
    if [[ "${DEPS_PRESENT}" == *ok* && "${FORCE_PIP:-0}" -eq 0 ]]; then
        log "venv deps already importable (${PC2_PIP_DEPS}); skipping pip"
        log "  (pass --force-pip to re-run the pip install)"
    else
        log "installing venv deps: ${PC2_PIP_DEPS}"
        ssh_pc2 "'${PC2_PREFIX}/venv/bin/pip' install --upgrade --quiet --disable-pip-version-check pip wheel"
        ssh_pc2 "'${PC2_PREFIX}/venv/bin/pip' install --quiet --disable-pip-version-check ${PC2_PIP_DEPS}"
    fi
    ...
    ssh_pc2 "source /opt/ros/${ROS_DISTRO}/setup.bash && \
        ...
        ${PC2_PREFIX}/venv/bin/python3 -c 'import rclpy, zmq, aimdk_msgs, onnxruntime, joblib, pygame, msgpack, scipy.spatial; print(\"venv ok\", zmq.__version__)'"
fi
...
if [[ "${SKIP_SOURCE}" -eq 1 ]]; then
    ...
    # x2_pose_watchdog.py does
    # `from gear_sonic.utils.pose_pipeline.{wire,fallback} import ...`.
...
if [[ "${SKIP_SOURCE}" -eq 1 ]]; then
    ...
    # Gamepad-ritual ignition scripts (x2_pc2/). These live at the TOP of
    # ${PC2_PREFIX} (not under gear_sonic_deploy/) because the pad
    # autopair daemon's --start-cmd and the pad bridge's chord handler
    # reference them there. They were originally staged by hand, which
    # let the PC2 copies drift from the repo; staging them here makes
    # the repo the source of truth (2026-07-29 blocked-ignition
    # incident: a stale hand-staged chain pointed the ritual at a
    # laptop-regenerated deploy body missing --no-confirm).
    # start_x2_deploy_ritual.sh is the deploy launcher the rituals use
    # INSTEAD of the laptop-autogenerated log/start_x2_deploy.sh.
    log "rsync x2_pc2/ ritual ignition scripts -> ${PC2_PREFIX}/"
    for ritual_script in \
        ritual_start_demo.sh \
        ritual_start_sonic.sh \
        start_x2_deploy_ritual.sh \
        start_x2_pose_watchdog_local.sh \
        pad_bindings.env \
        ; do
        if [[ ! -f "${REPO_ROOT}/x2_pc2/${ritual_script}" ]]; then
            warn "ritual script missing on laptop: x2_pc2/${ritual_script} -- skipping"
            continue
        fi
        rsync_to_pc2 \
            "${REPO_ROOT}/x2_pc2/${ritual_script}" \
            "${PC2_PREFIX}/${ritual_script}"
    done

    # robot_env.env is the ritual's single env file (model paths, tuning
    # preset, planner knobs). It is per-robot, so the repo ships only
    # templates: copy x2_pc2/robot_env.env.template (dual-head set) or
    # x2_pc2/robot_env.env.frozen.template (frozen-G1 token set) to
    # x2_pc2/robot_env.env, point the X2_RITUAL_MODEL / X2_WB_* lines at
    # your ONNX files under ${PC2_PREFIX}/policies/, and re-run bringup. The
    # template as shipped already names the default set step 9 stages.
    if [[ -f "${REPO_ROOT}/x2_pc2/robot_env.env" ]]; then
        log "rsync x2_pc2/robot_env.env -> ${PC2_PREFIX}/robot_env.env"
        rsync_to_pc2 "${REPO_ROOT}/x2_pc2/robot_env.env" "${PC2_PREFIX}/robot_env.env"
    else
        warn "x2_pc2/robot_env.env missing on laptop -- the ritual REFUSES to launch without it on PC2."
        warn "    cp x2_pc2/robot_env.env.template x2_pc2/robot_env.env   # (or robot_env.env.frozen.template)"
        warn "    (the template points at the shipped default set; edit the X2_RITUAL_MODEL /"
        warn "     X2_WB_TOKEN_MODEL / X2_WB_TOKENIZER lines only for another model) and re-run."
    fi
fi

# -------------------------------------------------------------------------
# Step 7b: gear_sonic python modules the PC2-side scripts import
# -------------------------------------------------------------------------
#
# pad_locomotion_bridge.py imports estop_gesture (standalone, beside it);
# pc2_pico_token_service.py puts ${PC2_PREFIX} on sys.path and imports
# gear_sonic.utils.teleop.{smpl_obs,zmq.zmq_planner_sender,
# zmq.zmq_packed_message_decoder}; pc2_kplanner_onnx.py resolves
# gear_sonic.utils.teleop.x2_hand_retarget + pose_pipeline.wire against
# ${PC2_PREFIX}/planner_stack (its PYTHONPATH in the rituals). Stage the
# three utils packages (py files only -- no tests/vis/vr/webxr assets) under
# BOTH prefixes so every import path the rituals use resolves. The same
# files are listed in x2_pc2/push_manifest*.txt so incremental pushes keep
# them current.

step "7b. gear_sonic python modules -> ${PC2_PREFIX}/{gear_sonic,planner_stack/gear_sonic}/"

if [[ "${SKIP_SOURCE}" -eq 1 ]]; then
    skip "--skip-source"
else
    for stack_root in "${PC2_PREFIX}/gear_sonic" "${PC2_PREFIX}/planner_stack/gear_sonic"; do
        ssh_pc2 "mkdir -p '${stack_root}/utils'"
        scp_to_pc2 "${REPO_ROOT}/gear_sonic/__init__.py" "${stack_root}/__init__.py"
        scp_to_pc2 "${REPO_ROOT}/gear_sonic/utils/__init__.py" "${stack_root}/utils/__init__.py"
        for pkg in teleop pose_pipeline planner; do
            log "rsync gear_sonic/utils/${pkg}/ (py only) -> ${stack_root#${PC2_PREFIX}/}/utils/${pkg}/"
            rsync_to_pc2 \
                "${REPO_ROOT}/gear_sonic/utils/${pkg}/" \
                "${stack_root}/utils/${pkg}/" \
                --exclude=__pycache__/ --exclude='test_*.py' \
                --exclude=vis/ --exclude=vr/ --exclude=solver/ \
                --include='*/' --include='*.py' --exclude='*'
        done
    done
    log "estop_gesture.py -> ${PC2_PREFIX}/ (standalone import beside pad_locomotion_bridge.py)"
    scp_to_pc2 "${REPO_ROOT}/gear_sonic/utils/teleop/estop_gesture.py" "${PC2_PREFIX}/estop_gesture.py"

    # systemd units + their installers (run ON PC2 with sudo; see step 10).
    log "rsync x2_pc2/*.service + install_*_service.sh -> ${PC2_PREFIX}/x2_pc2/"
    for unit_file in x2-pad-autopair.service x2-camera-bridge.service \
                     install_pad_autopair_service.sh install_camera_bridge_service.sh; do
        if [[ -f "${REPO_ROOT}/x2_pc2/${unit_file}" ]]; then
            scp_to_pc2 "${REPO_ROOT}/x2_pc2/${unit_file}" "${PC2_PREFIX}/x2_pc2/${unit_file}"
        else
            warn "x2_pc2/${unit_file} missing on laptop -- skipping"
        fi
    done
fi
...
# transforms). Falls back to the committed pre-baked file when the source
# PKL isn't on the laptop; a bake that ERRORS aborts bringup rather than
# silently rsyncing a stale clip.
...
if [[ ! -f "${IDLE_PRIMS_PKL}" ]]; then
    if [[ -f "${IDLE_X2M2_LOCAL}" ]]; then
        ...
        warn "    Generate the primitives PKL with tools/build_demo_bank_from_upstream.sh"
        warn "    (see manifest/REGENERATE.md), then re-run bringup,"
    ...
    # uses joblib (PKL load) + scipy (resample + yaw align). The
    # laptop's .venv has both; PC2 venv does not need them since the
    # proxy on PC2 loads the baked X2M2 directly (numpy-only).
    # $PYTHON overrides (same convention as tools/build_demo_bank_from_upstream.sh).
    BAKER_PY="${PYTHON:-${REPO_ROOT}/.venv/bin/python}"
    if [[ ! -x "${BAKER_PY}" ]]; then
    ...
    if [[ "${DRY_RUN}" -eq 1 ]]; then
        ...
        if ! (
            ...
            fail "bake_idle_stand_x2m2 failed (python: ${BAKER_PY}); refusing to stage a stale idle_stand.x2m2.
       Needs numpy + scipy + joblib and gear_sonic importable from ${REPO_ROOT}
       (e.g. the repo .venv). Fix the bake, or re-run with the primitives PKL
       removed to fall back to the committed gear_sonic_deploy/data/idle_stand.x2m2."
        fi
    fi
fi
...
# -------------------------------------------------------------------------
# Step 8b: demo bank (.x2m2 clips) + kplanner idle anchor -> planner_stack/models/
# -------------------------------------------------------------------------
#
# The rituals launch pc2_kplanner_onnx.py with
#   --warmup-qpos planner_stack/models/kplanner_idle_anchor_g1teleop_v3.pkl
#   --dances-dir  planner_stack/models/dances_x2m2
# and the pad bridge binds its clip banks (x2_pc2/pad_bindings.env) to
# <key>.x2m2 files in that dir. pc2_kplanner_onnx.py additionally resolves
# planner_stack/models/x2_planner_primitives.pkl (lean / crouch / torso
# primitives; the planner runs without it, those pad gestures just log a
# warning). Nothing else stages them.

step "8b. demo bank + idle anchor -> ${PC2_PREFIX}/planner_stack/models/"

DEMO_BANK_LOCAL="${REPO_ROOT}/gear_sonic_deploy/data/motions_x2m2/demo_bank"
IDLE_ANCHOR_LOCAL="${REPO_ROOT}/gear_sonic/data/motions/kplanner_idle_anchor_g1teleop_v3.pkl"
DANCES_REMOTE="${PC2_PREFIX}/planner_stack/models/dances_x2m2"

if compgen -G "${DEMO_BANK_LOCAL}/*.x2m2" >/dev/null; then
    log "rsync demo bank ($(ls "${DEMO_BANK_LOCAL}"/*.x2m2 | wc -l) clips) -> ${DANCES_REMOTE}/"
    ssh_pc2 "mkdir -p '${DANCES_REMOTE}'"
    # No --delete: operator-staged extra clips (run_motion_replay.sh) survive.
    if [[ "${DRY_RUN}" -eq 1 ]]; then
        plan "rsync -av ${DEMO_BANK_LOCAL}/*.x2m2 ${PC2_TARGET}:${DANCES_REMOTE}/"
    else
        rsync -av --info=stats0,progress0 "${DEMO_BANK_LOCAL}"/*.x2m2 "${PC2_TARGET}:${DANCES_REMOTE}/"
    fi
else
    warn "no demo bank at gear_sonic_deploy/data/motions_x2m2/demo_bank/*.x2m2 -- the pad clip banks will be EMPTY on PC2."
    warn "    Generate it first:  tools/build_demo_bank_from_upstream.sh   (manifest/REGENERATE.md), then re-run bringup."
fi

if [[ -f "${IDLE_ANCHOR_LOCAL}" ]]; then
    if head -c 40 "${IDLE_ANCHOR_LOCAL}" | grep -q 'git-lfs'; then
        warn "kplanner_idle_anchor_g1teleop_v3.pkl is an LFS pointer -- run 'git lfs pull' first; skipping"
    else
        log "rsync kplanner_idle_anchor_g1teleop_v3.pkl -> planner_stack/models/"
        rsync_to_pc2 "${IDLE_ANCHOR_LOCAL}" "${PC2_PREFIX}/planner_stack/models/kplanner_idle_anchor_g1teleop_v3.pkl"
    fi
else
    warn "gear_sonic/data/motions/kplanner_idle_anchor_g1teleop_v3.pkl missing on laptop -- pc2_kplanner_onnx.py --warmup-qpos will fail; skipping"
fi

PRIMS_LOCAL="${REPO_ROOT}/gear_sonic/data/motions/x2_planner_primitives.pkl"
if [[ -f "${PRIMS_LOCAL}" ]]; then
    if head -c 40 "${PRIMS_LOCAL}" | grep -q 'git-lfs'; then
        warn "x2_planner_primitives.pkl is an LFS pointer -- regenerate it with tools/build_demo_bank_from_upstream.sh; skipping"
    else
        log "rsync x2_planner_primitives.pkl -> planner_stack/models/"
        rsync_to_pc2 "${PRIMS_LOCAL}" "${PC2_PREFIX}/planner_stack/models/x2_planner_primitives.pkl"
    fi
else
    warn "gear_sonic/data/motions/x2_planner_primitives.pkl missing on laptop -- kplanner lean/crouch/torso primitives will be UNAVAILABLE on PC2."
    warn "    Generate it first:  tools/build_demo_bank_from_upstream.sh   (manifest/REGENERATE.md), then re-run bringup."
fi
...
step "9. ONNX model checkpoint (--model, default: shipped set)"

# The shipped default set (gear_sonic_deploy/models/README.md): the four
# graphs x2_pc2/robot_env.env.template points X2_RITUAL_MODEL / X2_WB_* at.
# They are git-lfs objects; a clone without `git lfs pull` holds ~130-byte
# pointer files, which must never land in policies/ (the deploy would fail
# to load them at ignition), so pointers are detected and skipped loudly.
...
if [[ "${SKIP_MODEL}" -eq 1 ]]; then
    ...
    log "no --model passed -> staging the shipped default set from gear_sonic_deploy/models/"
    for m in "${SHIPPED_MODELS[@]}"; do
        if [[ ! -f "${SHIPPED_MODELS_DIR}/${m}" ]]; then
            warn "shipped model missing on laptop: gear_sonic_deploy/models/${m} -- skipping"
        elif head -c 40 "${SHIPPED_MODELS_DIR}/${m}" | grep -q 'git-lfs'; then
            warn "gear_sonic_deploy/models/${m} is an LFS pointer -- run 'git lfs pull' first; skipping"
        else
            log "rsync ${m} -> ${PC2_PREFIX}/policies/"
            rsync_to_pc2 "${SHIPPED_MODELS_DIR}/${m}" "${PC2_PREFIX}/policies/${m}"
        fi
    done
    skip "another model: --model <file.onnx>, or x2_pc2/push_to_pc2.sh with an edited manifest"
else
...
printf '\n'
printf '    3. Push the planner/pad scripts + models (md5-gated):\n\n'
printf '         x2_pc2/push_to_pc2.sh --manifest x2_pc2/push_manifest.txt --pc2 %s\n' "${PC2_TARGET}"
printf '\n'
printf '    4. ONCE, on PC2 (sudo): pad driver + boot ritual service:\n\n'
printf '         ssh %s "bash %s/gear_sonic_deploy/scripts/pc2_pad_setup.sh && bash %s/x2_pc2/install_pad_autopair_service.sh"\n' "${PC2_TARGET}" "${PC2_PREFIX}" "${PC2_PREFIX}"
printf '\n'
printf '    5. Or start a laptop-driven deploy:\n\n'
printf '         gear_sonic_deploy/scripts/x2_pc2_daemons.sh start \\\n'
printf '             --model %s/policies/x2_sonic_v16ft8_45000_g1.onnx \\\n' "${PC2_PREFIX}"
printf '             --tuning gear_sonic_deploy/configs/real_deploy_tuning/trained_gains_s0_inc_waistmc.yaml\n'
printf '\n'
